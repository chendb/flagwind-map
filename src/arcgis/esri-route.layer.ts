    ...
    export class EsriRouteLayer extends FlagwindRouteLayer {
               ...
               public onSetSegmentByPoint(options: any, segment: TrackSegment) {
                   ...
                   for (let i = 0; i < points.length - 1; i++) {
                       ...
                       const tmppolyline = new esri.geometry.Polyline(this.flagwindMap.spatial).addPath(
                           [start, end]
                       );
               ...
               public onShowSegmentLine(segment: TrackSegment) {
                   let playedLineSymbol = new esri.symbol.CartographicLineSymbol(esri.symbol.CartographicLineSymbol.STYLE_SOLID, new esri.Color(
                           [48, 254, 62, 0.8]
                       ), 4, esri.symbol.CartographicLineSymbol.CAP_ROUND, esri.symbol.CartographicLineSymbol.JOIN_MITER, 2); // 38, 101, 196, 0.8
               ...
               public onCreateMoveMark(trackline: TrackLine, graphic: any, angle: number) {
                   ...
                   let marker = new esri.Graphic(
                       graphic.geometry,
                       symbol,
                       { __type: "point", __line: trackline.name }
                   );
               ...
               public onCreateLineLayer(id: string): FlagwindGroupLayer {
                   return new EsriGroupLayer({ id: id });
               }
               ...
               public onSolveByService(segment: TrackSegment, start: any, end: any, waypoints: Array<any>): void {
                   ...
                   routeTask.on("solve-complete", function(
                       evt: any
                   ) {
                       const routeResult =
                           evt.result.routeResults[0];
