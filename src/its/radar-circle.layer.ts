...
import { ControlCircleLayer } from './control-circle.layer';
...
import { LightingAnimation, AnimationLayer } from './animation.model';
...
export class RadarTollgateLayer extends TollgateLayer {
    constructor(flagwindMap: FlagwindMap, id: string, options: object) {
        ...
        this.animationLayer = new AnimationLayer({
            ...
            getColor: function getColor(item: any) {
    ...
    updateAnimation(item: any, graphic: any) {
        const animation: any = this.animationLayer.getAnimationId(item.id);
        if (animation) {
...
export class RadarCircleLayer {
    ...
    constructor(flagwindMap, id, options) {
        this.flagwindMap = flagwindMap;
        this.id = id;
        this.options = Object.assign({}, radarLayerOptions, options);
        const _this = this;
        this.circleLayer = new ControlCircleLayer(flagwindMap, (id || "radar") + "_circle", {
            showInfoWindow: true,
        ...
        this.tollgateLayer = new RadarTollgateLayer(flagwindMap, (id || "radar") + "_tollgate", {
