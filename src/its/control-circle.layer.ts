...
export class ControlCircleLayer extends DeviceLayer {
    ...
    */
    ...
    showFollowGraphic(item: any) {
        const me = this;
        (this.groupLayer.layer.graphics).forEach((g: any) => {
            if ((!item) || item.id == g.attributes.__name) {
                if (g.attributes.__type == "Polygon") {
                    g.setSymbol(me.getFillSymbol(g.attributes));
                    g.draw(); //重绘
                }
                if (g.attributes.__type == "Polyline") {
                    g.setSymbol(me.getLineSymbol(g.attributes));
                    g.draw(); //重绘
                }
        ...
        this.layer.graphics.forEach((g: any) => {
            if ((!item) || item.id == g.attributes.id) {
                g.setSymbol(me.getCentreSymbol(g.attributes));
                g.draw();
            } else {
                g.setSymbol(null);
                g.draw();
    ...
    updatePolygonGraphic(item: any) {
        ...
        this.groupLayer.getGraphicByName(item.id).forEach((g: any) => {
            if (g.attributes.__type == "Polygon") {
