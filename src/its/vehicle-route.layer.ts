import { FlagwindRouteLayer } from '../base/flagwind-route.layer';
 ...
 * 车辆路由图层
 */
export class VehicleRouteLayer extends FlagwindGroupLayer {
    moveLabelLayer: any;
    layerName: any;
    flagwindMap: FlagwindMap;
    moveMarkLayer: any;

    constructor(flagwindMap: FlagwindMap, layerName: any, options: any) {
        super(flagwindMap);
        // 移动的小车车牌
    }
    ...
    onAddLayerAfter() {
        // 移动的小车车牌
        this.moveLabelLayer = new FlagwindGroupLayer(this.layerName + "LabelLayer");
     ...
     * @param {*} angle 偏转角
     */
    createMoveMark(trackline: any, graphic: any, angle: any) {
     ...
     * 每次位置移动线路上的要素样式变换操作
     */
    changeMovingGraphicSymbol(trackline: any, point: any, angle: any) {
    ...
    createVehiclePlateMarker(trackline: any, graphic: any, angle: any) {
    ...
    getStandardStops(name: any, stops: any, stopSymbol: any) {
        const stopGraphics = [];
     ...
     * 修改路段坐标系
     * @param {*} segment 路段
     * @param {*} flagwindMap 地图修饰类
     */
    changeSegmentSpatial(segment: any, flagwindMap: FlagwindMap) {

        const polyline = new esri.geometry.Polyline(flagwindMap.spatial.wkid);
        for (let j = 0; j < segment.polyline.paths.length; j++) {
            let points = segment.polyline.paths[j].map((xy: any) => {
                let pt = this.flagwindMap.getPoint({
                    longitude: xy[0],
                    latitude: xy[1]
                });
                return [pt.x, pt.y];
        ...
        if (segment.line) {
            let line = segment.line.map((xy: any) => {
                let pt = flagwindMap.getPoint({
                    longitude: xy[0],
                    latitude: xy[1]
                });
                return [pt.x, pt.y];
        ...
        if (segment.startGraphic) {
            ...
            segment.startGraphic.setGeometry(flagwindMap.getPoint({
        ...
        if (segment.endGraphic) {
            ...
            segment.endGraphic.setGeometry(flagwindMap.getPoint({
        ...
        if (segment.waypoints) {
            segment.waypoints.forEach((g: any) => {
                let geometry = g.geometry;
                g.setGeometry(flagwindMap.getPoint({
     ...
     * @param {*} segment 
     */
    onCreateSegmentLineComplete(segment: any) {
        if (MapSetting.wkid == 3857 && MapSetting.is25D && MapSetting.wkidFromApp == 4326) {
            this.changeSegmentSpatial(segment, this.flagwindMap);
        }
    }
     ...
     * 线路上移动要素的构建（子）
     */
    getMarkerGraphic(trackline: any, graphic: any, angle: any) {
     ...
     * 线路上移动要素的上方label构建
     */
    getVehiclePlateGraphic(trackline: any, graphic: any, angle: any) {
    ...
    getVehiclePlateBgGraphic(trackline: any, graphic: any, angle: any) {
